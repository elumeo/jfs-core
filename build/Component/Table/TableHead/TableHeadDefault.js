...
const TableHeadDefault = ({ headerProps }) => {
    ...
    return (headerProps && (React.createElement(TableCell, { component: 'div', className: clsx(globalClasses.tableCell, globalClasses.flexContainer, classes.noClick), variant: 'head', style: { height: headerProps.columnData.headerHeight }, align: headerProps.columnData.numeric || false ? 'right' : 'left' },
        headerProps.disableSort !== true && (React.createElement(TableSortLabel, { active: headerProps.sortBy === headerProps.dataKey, direction: headerProps.sortBy === headerProps.dataKey
                ? mapSortDirection(headerProps.sortDirection)
                : 'asc' },
            headerProps.label,
            headerProps.sortBy === headerProps.dataKey ? (React.createElement("span", { className: classes.visuallyHidden }, headerProps.sortDirection.toLowerCase() === 'desc'
                ? 'sorted descending'
