...
const useStyles = makeStyles<Theme, AppCardContentProps>((theme) => createStyles({
  root: {
    display: 'flex',
...
const AppCardContent = (props: AppCardContentProps) => {
  const { fullHeight = false, withSubtitle = false, overrideCardTitleHeight = null, children, ...rest } = props;
  const classes = useStyles({ ...props, fullHeight, withSubtitle, overrideCardTitleHeight });

  return <CardContent classes={{ root: classes.root }} {...rest}>{children}</CardContent>;
};
