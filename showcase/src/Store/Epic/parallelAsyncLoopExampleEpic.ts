...
type ParallelAsyncLoopExampleData = { // just a demo interface (you have your own)
  factor: number;
  items: string[];
  progress: number;
};
...
export const parallelAsyncLoopExampleRequestActionEpic: Epic = action$ => {
  return action$.pipe(
    filter(isActionOf(Action.parallelAsyncLoopExampleRequestAction)),
    map((action) => { // preparation of data to use in the for loop
      return {
        factor: 100 / action.payload.length,
        items: action.payload,
        progress: 0
      } as ParallelAsyncLoopExampleData;
    }),
    concatMap((data) => { // here wo do our client calls in a loop
      ...
      return new Observable<ParallelAsyncLoopExampleData>((observer) => {
    ...
    switchMap((data) => iif(() =>
      ...
      of(Action.parallelAsyncLoopExampleUpdateAction(data.progress)), // progress below 100%
      of(
        Action.parallelAsyncLoopExampleUpdateAction(data.progress),
